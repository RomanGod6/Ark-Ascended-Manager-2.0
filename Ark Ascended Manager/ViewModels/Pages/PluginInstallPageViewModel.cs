...
public class PluginInstallPageViewModel : INotifyPropertyChanged
{
    ...
    public PluginInstallPageViewModel()
    {
        ...
        {
            FetchLatestPluginDetailsAsync(SelectedPlugin.ResourceId);
        }
        else
        {
            Debug.WriteLine("SelectedPlugin is null");
        }
    }
    ...
    public async Task FetchLatestPluginDetailsAsync(int resourceId)
    {
        ...
        try
        {
            Debug.WriteLine("Fetching plugin details...");
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                Debug.WriteLine($"Fetched plugin details: {SelectedPlugin?.Title}");
            }
            else
            {
    ...
    public BasicResource LoadCurrentPluginData()
    {
        ...
        if (File.Exists(currentPluginPath))
        {
            ...
            SelectedPlugin = JsonConvert.DeserializeObject<BasicResource>(json);
            Debug.WriteLine($"Loaded plugin: {SelectedPlugin?.Title}");
            return SelectedPlugin; // Ensure you're setting the property
        }
        else
        {
    ...
    public BasicResource SelectedPlugin
{
    ...
    set
    {
        if (_selectedPlugin != value)
        {
            ...
            OnPropertyChanged(nameof(SelectedPlugin));
            Debug.WriteLine($"SelectedPlugin set: Title={_selectedPlugin?.Title}, ImagePath={_selectedPlugin?.ImagePath}, Description={_selectedPlugin?.Description}");
        }
    }
}
